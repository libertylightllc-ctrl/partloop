...
export default async function SearchPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const results = await searchMarketplaceProducts({ q, category, condition, compatibility, verified: sellerVerified, sort });
    ...
    <main className="page-shell search-page">
      <div className="search-header">
        <div>
          ...
          <h1>{q ? (ar ? `نتائج “${q}”` : `Results for “${q}”`) : (ar ? "كل قطع السيارات" : "All auto parts")}</h1>
      ...
      <div className="search-layout">
        <form className="filters" method="get">
          ...
          <label>{ar ? "السيارة" : "Vehicle"}<select name="vehicle" defaultValue={String(params.vehicle ?? "")}><option value="">{ar ? "كل السيارات" : "All vehicles"}</option><option value="toyota-land-cruiser">Toyota Land Cruiser 2021</option><option value="nissan-patrol">Nissan Patrol 2019</option></select></label>
          <label>{ar ? "الفئة" : "Category"}<select name="category" defaultValue={category}><option value="">{ar ? "كل الفئات" : "All categories"}</option><option value="lights">{ar ? "إضاءة" : "Lights"}</option><option value="engines">{ar ? "محركات" : "Engines"}</option><option value="body">{ar ? "هيكل" : "Body parts"}</option><option value="transmission">{ar ? "ناقل الحركة" : "Transmission"}</option><option value="wheels">{ar ? "جنوط" : "Wheels"}</option></select></label>
